...
class ItemList extends Component {
  ...
  addItem = newItem => {
    ...
    // if sku already exists in array, accrue qty
  ...
  // pass newValue & currentIndex up here, from Item.js, from Integerinput.js, via param instead of e.target.value
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.state.itemlist !== prevState.itemlist) {
      this.props.updateItemListInOrder(this.state.itemlist); // update parent's state, mk sure it sync with CreateOrder
    }
  }

  render() {
    // var str1 = JSON.stringify(this.state.itemlist);
    // console.log("ItemList:  " + str1);
    // var str2 = JSON.stringify(this.props.itemlist);
    // console.log("CreateOrder:  " + str2);
